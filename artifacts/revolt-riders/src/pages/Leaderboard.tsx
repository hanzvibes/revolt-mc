...
function parseJoinDate(bergabung: string): Date | null {
  if (!bergabung) return null;
  const yearMatch = bergabung.match(/\d{4}/);
  if (!yearMatch) return null;
  const year = parseInt(yearMatch[0], 10);
  const monthNames = ["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"];
  const lower = bergabung.toLowerCase();
  const mIdx = monthNames.findIndex((m) => lower.includes(m));
  return new Date(year, mIdx >= 0 ? mIdx : 6, 1);
}

function computePaceData(rider: { bergabung: string; total_km: number }, avgKmPerMonth: number) {
  const joinDate = parseJoinDate(rider.bergabung);
  if (!joinDate) return null;
  const now = new Date();
  const monthsActive = Math.max(1,
    (now.getFullYear() - joinDate.getFullYear()) * 12 + (now.getMonth() - joinDate.getMonth())
  );
  const kmPerMonth = Math.round(rider.total_km / monthsActive);
  const nextMilestone = MILESTONES.find((m) => m > rider.total_km) ?? null;
  const monthsToMilestone = nextMilestone && kmPerMonth > 0
    ? Math.ceil((nextMilestone - rider.total_km) / kmPerMonth)
    : null;
  const status: "ahead" | "on-track" | "behind" =
    avgKmPerMonth > 0
      ? kmPerMonth >= avgKmPerMonth * 1.2 ? "ahead"
      : kmPerMonth <= avgKmPerMonth * 0.7 ? "behind"
      : "on-track"
    : "on-track";
  return { kmPerMonth, monthsActive, nextMilestone, monthsToMilestone, status };
}
...
/* ── Pace chip ────────────────────────────────────────────────── */
function PaceChip({ pace }: { pace: ReturnType<typeof computePaceData> }) {
  if (!pace) return null;
  const cfg = {
    ahead:     { label: "AHEAD",    cls: "bg-green-500/10 text-green-400 border-green-500/20" },
    "on-track":{ label: "ON TRACK", cls: "bg-blue-500/10 text-[#60A5FA] border-blue-500/20" },
    behind:    { label: "BEHIND",   cls: "bg-red-500/10 text-red-400 border-red-500/20" },
  }[pace.status];
  return (
    <div className="hidden md:flex items-center gap-2 mt-1.5 flex-wrap">
      <span className="text-[0.6rem] text-[#52525B]">~{pace.kmPerMonth.toLocaleString("id-ID")} km/mo</span>
      {pace.nextMilestone !== null && pace.monthsToMilestone !== null && pace.monthsToMilestone <= 36 && (
        <>
          <span className="text-[0.52rem] text-[#3F3F46]">·</span>
          <span className="text-[0.6rem] text-[#52525B]">
            {pace.nextMilestone.toLocaleString("id-ID")} km in ~{pace.monthsToMilestone}mo
          </span>
        </>
      )}
      <span className={cn("text-[0.55rem] font-bold tracking-widest px-1.5 py-0.5 rounded-md border uppercase", cfg.cls)}>
        {cfg.label}
      </span>
    </div>
  );
...
export default function Leaderboard({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const avgKmPerMonth = useMemo(() => {
    ...
    const paces = riders
      .map((r) => {
        const joinDate = parseJoinDate(r.bergabung);
        if (!joinDate) return null;
        const now = new Date();
        const months = Math.max(1,
          (now.getFullYear() - joinDate.getFullYear()) * 12 + (now.getMonth() - joinDate.getMonth())
        );
        return Math.round(r.total_km / months);
      })
      .filter((p): p is number => p !== null);
    return paces.length > 0 ? Math.round(paces.reduce((s, p) => s + p, 0) / paces.length) : 0;
  }, [riders]);
          ...
          {loading ? (
            ...
            <>
              ...
              {riders.slice(0, 3).map((rider, i) => (
                <RiderRow key={rider.id} rider={rider} index={i} maxKm={maxKm} avgKmPerMonth={avgKmPerMonth} isTop3 />
              ))}
              ...
              {riders.length > 3 && (
                <div className="flex items-center gap-3 px-5 py-2 bg-[rgba(255,255,255,0.015)] border-y border-[rgba(39,39,42,0.4)]">
                  ...
                  <span className="text-[0.62rem] font-medium uppercase tracking-widest text-[#52525B]">Chasing the podium</span>
                  <div className="h-px flex-1 bg-[rgba(39,39,42,0.8)]" />
                </div>
              )}
              ...
              {/* Rest */}
              {rest.map((rider, i) => (
                <RiderRow key={rider.id} rider={rider} index={i + 3} maxKm={maxKm} avgKmPerMonth={avgKmPerMonth} />
              ))}
...
/* ── Rider row ──────────────────────────────────────────────────── */
function RiderRow({ rider, index, maxKm, avgKmPerMonth, isTop3 = false }: {
  rider: RankedRider; index: number; maxKm: number; avgKmPerMonth: number; isTop3?: boolean;
}) {
  const pace = computePaceData(rider, avgKmPerMonth);
  const rankColors = {
