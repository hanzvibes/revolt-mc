...
import BadgeDropOverlay, { type BadgeDrop } from "@/components/BadgeDropOverlay";

const BADGE_SEEN_KEY = "rr_seen_badges_v2";

function getSeenBadges(): Record<string, string[]> {
  try { return JSON.parse(localStorage.getItem(BADGE_SEEN_KEY) ?? "{}"); }
  catch { return {}; }
}

function markBadgeSeen(riderId: string, badgeId: string) {
  const seen = getSeenBadges();
  seen[riderId] = [...new Set([...(seen[riderId] ?? []), badgeId])];
...
export default function Trophies({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const [dropQueue, setDropQueue] = useState<BadgeDrop[]>([]);
  const [currentDrop, setCurrentDrop] = useState<BadgeDrop | null>(null);
  ...
  useEffect(() => {
    if (loading || riders.length === 0 || hasCheckedBadges.current) return;
    hasCheckedBadges.current = true;
    const seen = getSeenBadges();
    const newDrops: BadgeDrop[] = [];
    riders.forEach((rider) => {
      const earned = computeBadges(rider);
      const seenIds = new Set(seen[rider.id] ?? []);
      earned.forEach((badge) => {
        if (!seenIds.has(badge.id)) {
          newDrops.push({ badge, riderName: rider.nama });
          markBadgeSeen(rider.id, badge.id);
        }
      });
    });
    if (newDrops.length > 0) {
      newDrops.sort((a, b) => {
        const order = { legendary: 0, gold: 1, special: 2, silver: 3, bronze: 4 };
        return order[a.badge.tier] - order[b.badge.tier];
      });
      setDropQueue(newDrops.slice(1));
      setCurrentDrop(newDrops[0]);
    }
  }, [loading, riders]);

  function handleDropDismiss() {
    if (dropQueue.length > 0) {
      setCurrentDrop(dropQueue[0]);
      setDropQueue((q) => q.slice(1));
    } else {
      setCurrentDrop(null);
    }
  }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-auto">
      <BadgeDropOverlay current={currentDrop} onDismiss={handleDropDismiss} />
      <Topbar
        title="Trophy Wall"
